...
public class PlayerHealth : MonoBehaviour, IDamageable
{
    ...
    private void Die()
    {
        ...
        // Fallback: Ensure GameOverScreen is triggered even if event subscription failed
        // (e.g., if GameOverScreen GameObject was disabled and couldn't subscribe)
        if (!GameOverScreen.IsGameOver)
        {
            Debug.LogWarning("PlayerHealth: GameOverScreen did not respond to event. Searching directly.");
            GameOverScreen gameOverScreen = FindFirstObjectByType<GameOverScreen>(FindObjectsInactive.Include);
            if (gameOverScreen != null)
            {
                gameOverScreen.gameObject.SetActive(true); // Ensure it's active
                gameOverScreen.ShowGameOver();
            }
            else
            {
                Debug.LogError("PlayerHealth: No GameOverScreen found in scene! Add a GameOverScreen component to show the game over UI.");
            }
        }
